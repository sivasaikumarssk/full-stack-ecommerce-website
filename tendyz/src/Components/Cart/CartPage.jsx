...
import { remProduct } from "../../Redux/Action";

export const CartDetails = (()=>{
        
    const addArr = JSON.parse(localStorage.getItem("quantitycart")) ||[];
    let mrp = [];
    const items = useSelector((state) => state.allProd.product);
    ...
    const handleButton = (value, e) => {
        if (value === 1) {
            ...
            addArr.push(items);
            localStorage.setItem("quantitycart",JSON.stringify(addArr));
            mrp.push(items.price);
           
        } else {
            dispatch(remProduct(e))
            addArr.pop();
            localStorage.setItem("quantitycart",JSON.stringify(addArr));
    ...
    const removeFunc = (id) => {
        ...
        let newCart = cartArr.filter((e)=>{ 
            return e.id !== id;
        })
        // console.log("cartArr2",newCart)
   ...
   const [newItems,setnewItems]  = useState([...cartArr]);

    if(cartArr.length < 1){
        return(
            <div className="emptycart-image">
            <img src="https://imgs.search.brave.com/GnnmxE3vVcw6VVfvHXuu90Yi07jDuZlXdaXo-_Adknw/rs:fit:461:225:1/g:ce/aHR0cHM6Ly90c2Uy/Lm1tLmJpbmcubmV0/L3RoP2lkPU9JUC5u/ekZqUFdQYmg5blJa/UzFsQmItVmF3QUFB/QSZwaWQ9QXBp" alt="" />
            
            <div>
            <Link to="/clothing"><button className="remove_btn">Back to shopping</button></Link>
            </div>
            </div>
        )
    }
    ...
    return (
        ...
        {newItems.map((e)=>{
            ...
            const cartFunc = ()=>{
                ...
                //   cartCount = addcartArr.length
                addcartArrC.push(cartArr.length);
                 console.log("mrp",addcartArrC);
            ...
            return(
                ...
                <div className="cart_div">
                    ...
                    <h2>{e.title}</h2> <br />
                    <h3>Price : ${e.price}</h3> <br />
                    <br /> <br />
                    <button onClick={()=> handleButton(-1, e)}> - </button>
                    <button onClick={()=> handleButton(1, e)}> + </button>
                    <button className="remove_btn" onClick={()=>removeFunc(e.id)}>
                        Remove
                    </button>
                    <br />
                    <br />
          ...
          <h3>Total number of items = {cartArr.length+addArr.length}</h3>
          <h3>Total cost = $ {mrp.reduce((acc,cv)=>{
        ...
        <div className="cards">
                 ...
                 <Link to="/checkout">
                    <button className="checkout_btn">
                        Amount to be Paid = $ {mrp.reduce((acc,cv)=>{
                         return acc+Number(cv)
                        },0).toFixed(2) }
                    </button>
                </Link> 
