...
export const CheckoutPage=()=>{
 ...
 const handlesubmit=(e)=>{
  e.preventDefault();
  axios.post("https://tendyz-data.herokuapp.com/users",{
      name:form.name,
...
const alertFunc = ()=>{
        if(form.name.length === 0 ){
            alert("Enter Name on the card")
        }if(form.Cardnumber.length < 8 ){
            alert("card number should have 8 digits")
        }
        else{
    ...
    return(
       <>
          ...
          <h3>Select a payment method</h3>
            ...
            <button className="continue" onClick={alertFunc}>Continue</button>
